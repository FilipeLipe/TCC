...
def valida_link(links_processados: set(), links_com_erro: set(), link: str) -> bool:
    ...
    regex_autenticacao = re.compile(r"(minha.ufop|minhaufop|proad|facebook|instagram|play.google|twitter|linkedin)", re.IGNORECASE)
    regex_absurdos = re.compile(r"(calendario|radio|revistacuringa|eventos|noticias|repositorio|editais|edital)", re.IGNORECASE)
    ...
    if regex_ufop.search(link) and regex_http_https.search(link):
        if not (regex_extensoes.search(link) or regex_palavras.search(link) or regex_barra_dupla.search(link) or regex_autenticacao.search(link) or regex_absurdos.search(link)):
